...
test("documents Mermaid scenario stays inside its interaction budgets", async ({
  ...
  if (!coldSample || warmSamples.length === 0)
    throw new Error("MERMAID-BENCH-E001 missing samples");
  const metrics = {
    coldFirstReadyMs: coldSample.firstReadyMs,
    coldPreparedToReadyMs: coldSample.preparedToReadyMs,
    warmFirstReadyMedianMs: percentile(
      warmSamples.map((sample) => sample.firstReadyMs),
      0.5,
    ),
    warmFirstReadyP95Ms: percentile(
      warmSamples.map((sample) => sample.firstReadyMs),
      0.95,
    ),
    warmPreparedToReadyMedianMs: percentile(
      warmSamples.map((sample) => sample.preparedToReadyMs),
      0.5,
    ),
    warmPreparedToReadyP95Ms: percentile(
      warmSamples.map((sample) => sample.preparedToReadyMs),
      0.95,
    ),
  ...
  expect(metrics.coldFirstReadyMs).toBeLessThanOrEqual(baseline.budgets.coldFirstReadyMs);
  expect(metrics.coldPreparedToReadyMs).toBeLessThanOrEqual(baseline.budgets.coldPreparedToReadyMs);
  expect(metrics.warmFirstReadyMedianMs).toBeLessThanOrEqual(
    baseline.budgets.warmFirstReadyMedianMs,
  );
  expect(metrics.warmFirstReadyP95Ms).toBeLessThanOrEqual(baseline.budgets.warmFirstReadyP95Ms);
  expect(metrics.warmPreparedToReadyMedianMs).toBeLessThanOrEqual(
    baseline.budgets.warmPreparedToReadyMedianMs,
  );
  expect(metrics.warmPreparedToReadyP95Ms).toBeLessThanOrEqual(
    baseline.budgets.warmPreparedToReadyP95Ms,
  );
